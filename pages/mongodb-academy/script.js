...
let userProgress = JSON.parse(localStorage.getItem('mongoHubProgress')) || { completedLessons: [], completedQuizzes: [] };
        ...
        quiz: [
            {
                ...
                options: ['In tables with rows and columns', 'In JSON-like documents', 'In a simple text file', 'In a graph structure'],
                correct: 1
            }
        ]
    },
    {
        ...
        lessons: [
            ...
            },
            {
                ...
                `,
                defaultCode: 'db.products.insertOne({\n  name: "Monitor",\n  price: 300,\n  category: "Electronics"\n})'
            }
        ],
        quiz: [
            {
                ...
                options: ['db.users.find("age": 28)', 'db.users.find({ age: 28 })', 'db.users.select({ age: 28 })', 'db.users.get(age=28)'],
                correct: 1
            }
        ]
    },
        ...
        quiz: [
            {
                ...
                options: ['db.collection.pipeline()', 'db.collection.find()', 'db.collection.aggregate()', 'db.collection.group()'],
                correct: 2
            }
        ]
    }
...
function renderQuiz(mIndex) {
    ...
    quiz.forEach((q, i) => {
        ...
        html += \`
                ...
                <button onclick="checkAnswer('\${q.id}', \${mIndex}, \${i})" class="mt-4 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded text-sm font-medium transition-colors">
                    Submit Answer
                </button>
...
function runQuery() {
    ...
    if (!match) {
        appendTerminalOutput('', 'error-msg', 'TypeError: db.' + rawQuery.split('.')[1] + ' is not a function or syntax is invalid.');
        return;
    }
    ...
    try {
        ...
        appendTerminalOutput('', 'error-msg', 'SyntaxError: Invalid JSON or query arguments. ' + e.message);
    }
}
